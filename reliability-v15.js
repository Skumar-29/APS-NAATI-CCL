...
(() => {
  ...
  const qualityLabel=d=>{
    if(String(d?.qualityTier||'').includes('revalidated'))return ['reviewed','✓ V15 natural Hindi · revalidated'];
    if(String(d?.qualityTier||'').includes('rebuilt'))return ['reviewed','✓ V15 natural Hindi · rebuilt'];
    if(/owner/i.test(String(d?.reviewStatus||'')))return ['reviewed','✓ Owner-edited local version'];
    return ['reviewed','✓ V15 natural Hindi · study-ready'];
  };
  ...
  practice=function v15Practice(){
    ...
    return shell(`${header('Dialogue Practice','Quality-gated bilingual dialogue library')}
      <div class="info"><b>V15 study-ready library with dialogue-specific vocabulary.</b> All packaged dialogues remain rebuilt or revalidated for natural Hindi. Learning Mode now starts with a dialogue hub where students can learn the key vocabulary first or start the dialogue immediately. Player controls, recording, transcript toggle, speed and response-gap settings are unchanged.</div>
  ...
  function learningHubOverlay(){
    ...
    return `<div class="fullscreen dialogue-learning-hub-screen"><header class="top"><button data-action="close-learning-hub">← Dialogue library</button><div><strong>${esc(d.title)}</strong><span>Learning Mode preparation</span></div><div class="top-actions"><button class="player-settings-button" data-action="app-settings" aria-label="Open settings" title="Settings">⚙ <b>Settings</b></button></div></header>
      ...
      <section class="learning-hub-choice-grid"><article class="learning-choice vocab-choice"><div class="learning-choice-icon">Aa</div><small>STEP 1 · OPTIONAL PREPARATION</small><h2>Learn this dialogue’s key vocabulary</h2><p>Practise the important English ↔ Hindi terms selected from this dialogue before you interpret the segments.</p><div class="dialogue-vocab-progress-line"><b>${esc(progressText)}</b><div><i style="width:${pct}%"></i></div></div>${button(`${vocabLabel} →`,'start-dialogue-vocab','secondary wide',`data-id="${d.id}" ${count?'':'disabled'}`)}</article>
