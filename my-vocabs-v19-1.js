(function(){
'use strict';
const VERSION='My Vocabs V19.1';
const ONLINE_CACHE_KEY='apsMyVocabsOnlineAssistV1:hi';
const ONLINE_CACHE_TTL=1000*60*60*24*30;
...
async function translateOnline(text,target='hi'){
  ...
  try{
    const d=await fetchJson(`https://api.mymemory.translated.net/get?q=${encodeURIComponent(q)}&langpair=en%7C${encodeURIComponent(target)}`);
    const v=d?.responseData?.translatedText;if(usableTranslation(v))return String(v).trim();
  }catch{}
  try{
    const d=await fetchJson(`https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=${encodeURIComponent(target)}&dt=t&q=${encodeURIComponent(q)}`);
    const v=Array.isArray(d?.[0])?d[0].map(x=>x?.[0]||'').join('').trim():'';if(usableTranslation(v))return v;
  }catch{}
  return '';
}
async function onlineSynonyms(term){
  try{const d=await fetchJson(`https://api.datamuse.com/words?rel_syn=${encodeURIComponent(term)}&max=8`);return cleanList((Array.isArray(d)?d:[]).map(x=>x?.word).filter(Boolean)).filter(x=>normaliseSearchText(x)!==normaliseSearchText(term)).slice(0,6);}catch{return [];}
}
async function onlineDictionary(term){
  const q=String(term||'').trim();if(!q||q.split(/\s+/).length>4)return {definition:'',example:''};
  try{
    const d=await fetchJson(`https://api.dictionaryapi.dev/api/v2/entries/en/${encodeURIComponent(q)}`);
    for(const entry of (Array.isArray(d)?d:[]))for(const meaning of (entry.meanings||[]))for(const def of (meaning.definitions||[])){
      const definition=String(def.definition||'').trim();const example=String(def.example||'').trim();if(definition||example)return {definition,example,partOfSpeech:meaning.partOfSpeech||''};
    }
  }catch{}
  return {definition:'',example:''};
}
function capitalise(s){const t=String(s||'').trim();return t?t.charAt(0).toUpperCase()+t.slice(1):'';}
function teachingExample(term,dict,local){
  if(dict?.example&&dict?.definition)return `${capitalise(dict.example.replace(/[.!?]+$/,''))}. Here “${term}” means ${dict.definition.replace(/[.!?]+$/,'')}.`;
  if(dict?.example)return capitalise(dict.example);
  if(dict?.definition)return `${capitalise(term)} means ${dict.definition.replace(/[.!?]+$/,'')}.`;
  return local?.exampleEnglish||'';
}
async function onlineLookupProposal(term){
  const q=String(term||'').trim();if(!q||!navigator.onLine)return null;
  const cached=cachedOnline(q);if(cached)return {...cached,lookupSource:'Online assist · cached',online:true};
  const local=localLookupProposal(q);
  try{
    const [hindi,syn,dict]=await Promise.all([translateOnline(q,'hi'),onlineSynonyms(q),onlineDictionary(q)]);
    if(!hindi&&!syn.length&&!dict.definition&&!dict.example)return null;
    const exampleEnglish=teachingExample(q,dict,local);
    const exampleHindi=exampleEnglish?await translateOnline(exampleEnglish,'hi'):'';
    const data={
      hindi:hindi||local.hindi||'',
      suggestedSynonyms:cleanList([...(syn||[]),...(local.suggestedSynonyms||[])]).slice(0,8),
      exampleEnglish:exampleEnglish||local.exampleEnglish||'',
      exampleHindi:exampleHindi||local.exampleHindi||'',
      topic:local.topic||'community',lookupSource:'Online translation + dictionary',online:true
    };
    cacheOnline(q,data);return data;
  }catch{return null;}
}
async function lookupProposal(term){
  if(navigator.onLine){const online=await onlineLookupProposal(term);if(online)return online;}
  const local=localLookupProposal(term);return {...local,lookupSource:local.lookupSource||(navigator.onLine?'APS fallback · online unavailable':'APS offline fallback')};
}
...
async function applyLookup(record,{fillMissingOnly=true}={}){
  const p=await lookupProposal(record.english);record.manualFields=record.manualFields||{};
  const setIfAllowed=(field,value)=>{if(!value)return;if(record.manualFields[field])return;if(fillMissingOnly&&String(record[field]||'').trim())return;record[field]=value;};
  setIfAllowed('hindi',p.hindi||'');
  record.suggestedSynonyms=p.suggestedSynonyms||record.suggestedSynonyms||[];
  setIfAllowed('exampleEnglish',p.exampleEnglish||'');
  setIfAllowed('exampleHindi',p.exampleHindi||'');
...
function rowHtml(r){
  ...
  return `<tr data-my-row="${esc(r.id)}" data-search="${esc(normaliseSearchText(`${r.english} ${r.hindi} ${sourcesText(r)}`))}">
    ...
    <td><textarea class="my-cell my-readonly my-suggested" readonly title="Suggested automatically from online assist or APS fallback">${esc((r.suggestedSynonyms||[]).join(', '))}</textarea></td>
    ...
    <td class="my-row-actions"><button data-action="my-autofill" data-id="${esc(r.id)}" title="Refresh missing online details">↻ Retry</button><button data-action="my-play-one" data-id="${esc(r.id)}" title="Open in word player">▶</button><button class="danger-text" data-action="my-delete" data-id="${esc(r.id)}" title="Delete this personal row">Delete</button></td>
  </tr>`;
}
function networkBadge(){return navigator.onLine?'<span class="my-online"><i></i> Online assist</span>':'<span class="my-offline"><i></i> Offline fallback</span>';}
function workspace(){
  ...
  <section class="my-vocab-actions"><button class="primary" data-action="my-add-row">+ Add Row</button><button class="secondary" data-action="my-fill-missing" ${state.myVocabWorkspace.batchFill?'disabled':''}>${state.myVocabWorkspace.batchFill?'Filling…':'↻ Fill Missing Details'}</button><button class="secondary" data-action="my-play-filtered" ${list.filter(r=>r.english&&r.hindi).length?'':'disabled'}>▶ Play Filtered (${list.filter(r=>r.english&&r.hindi).length})</button><button class="secondary" data-action="my-export-csv">Export CSV</button><button class="secondary" data-action="my-import-trigger">Import CSV</button><input id="myImportCsv" type="file" accept=".csv,text/csv" hidden><span>${list.length.toLocaleString()} rows</span></section>
  <div class="my-sheet-wrap"><table class="my-sheet"><thead><tr><th>English</th><th>Hindi Meaning</th><th>Suggested Synonyms</th><th>My Synonyms</th><th>English Example</th><th>Hindi Example</th><th>Recall</th><th>Source</th><th>Actions</th></tr></thead><tbody>${list.map(rowHtml).join('')||`<tr><td colspan="9"><div class="my-empty"><h3>${rows().length?'No rows match these filters':'Your personal vocabulary sheet is empty'}</h3><p>${rows().length?'Change the search or recall filter.':'Type words quickly: Add Row → type English → Enter → next row.'}</p><button class="primary" data-action="my-add-row">+ Add your first word</button></div></td></tr>`}</tbody></table></div></main>${renderModal()}</div>`;
}
...
function openWorkspace(separate=true){
  ...
  captureReturnContext();stopAllSpeech();state.modal=null;state.overlay='my-vocabs';render();
}
...
function updateRowDom(r){
  ...
  const put=(f,v)=>{const el=row.querySelector(`[data-my-field="${f}"]`);if(el&&document.activeElement!==el)el.value=v||'';};
  put('hindi',r.hindi);put('exampleEnglish',r.exampleEnglish);put('exampleHindi',r.exampleHindi);
  const sg=row.querySelector('.my-suggested');if(sg)sg.value=(r.suggestedSynonyms||[]).join(', ');
  const src=row.querySelector('.my-source');if(src)src.innerHTML=`<span title="${esc(sourcesText(r))}">${esc(sourcesText(r))}</span>${r.lookupSource?`<small>Auto-filled from ${esc(r.lookupSource)}</small>`:''}`;
}
async function autoFillRecord(id,{showMessage=false}={}){
  const r=findMy(id);if(!r||!String(r.english||'').trim())return r;
  const before=normaliseSearchText(r.english);await applyLookup(r,{fillMissingOnly:true});if(before!==normaliseSearchText(findMy(id)?.english||r.english))return r;upsert(r);updateRowDom(r);
  if(showMessage)showToast(r.hindi?`Details refreshed · ${r.lookupSource||'APS'}`:'No online result found — offline APS fallback also had no exact match');return r;
}
...
async function fillMissingDetails(){
  const targets=filteredRows().filter(r=>String(r.english||'').trim()&&(!String(r.hindi||'').trim()||!String(r.exampleEnglish||'').trim()||!String(r.exampleHindi||'').trim()||!(r.suggestedSynonyms||[]).length));
  if(!targets.length){showToast('No missing details in the current filtered rows');return;}
  state.myVocabWorkspace.batchFill=true;render();let done=0;
  for(const r of targets){await autoFillRecord(r.id);done++;const note=document.querySelector('.my-vocab-actions .my-batch-progress');if(note)note.textContent=`${done}/${targets.length}`;}
  state.myVocabWorkspace.batchFill=false;render();showToast(`Filled available details for ${done} word${done===1?'':'s'}`);
}
...
function quickModal(){
  ...
  return `<div class="modal-backdrop"><div class="modal my-quick-modal"><button class="modal-close" data-action="close-modal">×</button><small>MY VOCABS · ${esc(sourceLabel(src))}</small><h2>Add a word without leaving this dialogue</h2><p>Type the English term. When online, APS uses internet translation/dictionary assistance first; offline APS vocabulary is the fallback.</p><label>English word<input id="myQuickEnglish" type="text" autocomplete="off" placeholder="e.g. eligible"></label><div class="my-quick-grid"><label>Hindi meaning<input id="myQuickHindi" type="text" placeholder="Auto-fill or type manually"></label><label>Suggested synonyms<input id="myQuickSuggested" type="text" readonly placeholder="Online / APS suggestions"></label></div><label>My synonyms<input id="myQuickSynonyms" type="text" placeholder="Your own synonyms, comma separated"></label><label>English example<textarea id="myQuickExampleEnglish" placeholder="Clear example that explains the meaning"></textarea></label><label>Hindi example<textarea id="myQuickExampleHindi" placeholder="सरल और स्पष्ट हिन्दी उदाहरण"></textarea></label><div id="myQuickLookupNote" class="my-lookup-note">Start typing an English word.</div><div class="actions"><button class="secondary" data-action="my-quick-autofill">↻ Retry</button><button class="primary" data-action="my-quick-save">Save to My Vocabs</button></div></div></div>`;
}
function helpModal(){return `<div class="modal-backdrop"><div class="modal my-quick-modal"><button class="modal-close" data-action="close-modal">×</button><small>MY VOCABS</small><h2>Fast vocabulary capture</h2><p><b>Online first:</b> when internet is available, APS uses online translation, synonym and dictionary assistance. If the internet or an online service is unavailable, installed APS vocabulary is used as the fallback.</p><p><b>Fast entry:</b> type an English word and press Enter. APS starts filling that row and moves you directly to a fresh English row.</p><p><b>Your edits stay yours:</b> Hindi, examples and My Synonyms that you type manually are not overwritten by automatic refreshes.</p><p><b>Privacy:</b> words sent for online assistance may be processed by third-party translation/dictionary services. My Vocabs itself remains in your APS progress storage/cloud sync.</p><div class="actions"><button class="primary" data-action="close-modal">Done</button></div></div></div>`;}
let quickLookupToken=0;
async function fillQuick(){
  const en=document.querySelector('#myQuickEnglish')?.value?.trim()||'',token=++quickLookupToken;if(!en)return;const note=document.querySelector('#myQuickLookupNote');if(note)note.textContent=navigator.onLine?'Looking up online…':'Offline — checking installed APS vocabulary…';
  const p=await lookupProposal(en);if(token!==quickLookupToken)return;
  const hi=document.querySelector('#myQuickHindi'),sg=document.querySelector('#myQuickSuggested'),ee=document.querySelector('#myQuickExampleEnglish'),eh=document.querySelector('#myQuickExampleHindi');
  ...
  if(note)note.textContent=p.hindi?`Found via ${p.lookupSource||'APS'}. You can edit any field before saving.`:'No result found. You can enter the Hindi meaning manually.';
}
async function saveQuick(){
  ...
  const r=newRecord(src);r.english=en;r.hindi=document.querySelector('#myQuickHindi')?.value?.trim()||'';r.suggestedSynonyms=cleanList(document.querySelector('#myQuickSuggested')?.value||'');r.mySynonyms=cleanList(document.querySelector('#myQuickSynonyms')?.value||'');r.exampleEnglish=document.querySelector('#myQuickExampleEnglish')?.value?.trim()||'';r.exampleHindi=document.querySelector('#myQuickExampleHindi')?.value?.trim()||'';r.manualFields={hindi:Boolean(r.hindi),mySynonyms:Boolean(r.mySynonyms.length),exampleEnglish:Boolean(r.exampleEnglish),exampleHindi:Boolean(r.exampleHindi)};const p=await lookupProposal(en);r.lookupSource=p.lookupSource||'';r.topic=p.topic||state.dialogue?.topic||'community';upsert(r);state.modal=null;render();showToast('Added to My Vocabs');
}
...
app.addEventListener('click',async event=>{
  ...
  else if(a==='my-quick-save'){event.preventDefault();await saveQuick();}
});
...
app.addEventListener('input',event=>{
  ...
  else if(t.id==='myQuickEnglish'){clearTimeout(searchTimer);searchTimer=setTimeout(()=>fillQuick(),300);}
});
app.addEventListener('keydown',async event=>{
  const t=event.target;if(!t?.matches?.('[data-my-field="english"]'))return;
...
if(new URL(location.href).searchParams.get('myvocabs')==='1'){
  ...
  const timer=setInterval(()=>{if(state.ready&&state.auth?.initialized&&state.selectedLanguage){clearInterval(timer);state.overlay='my-vocabs';document.documentElement.classList.add('my-vocabs-ready');render();}},60);
...
console.info(`${VERSION} loaded · online-first personal vocabulary · compact focus workspace · master APS content remains separate.`);
})();
