...
function assessAndSaveDialogue(){
  ...
  const attempt={id:`attempt-${Date.now()}`,dialogueId:d.id,title:d.title,mode:state.dialogueMode,startedAt:new Date(startedMs).toISOString(),finishedAt:new Date(finishedMs).toISOString(),finished:true,elapsedSeconds,repeats:state.repeats,responses:state.responses.map(r=>({...r,recordingUrl:''})),report};
