...
const storageKeys={onboard:'apsFinalOnboarded',vocabStatus:'apsFinalVocabStatus',vocabSettings:'apsFinalVocabSettings',vocabResume:'apsFinalVocabResume',attempts:'apsFinalAttempts',lesson:'apsFinalLesson',mistakes:'apsFinalMistakes',phraseStats:'apsFinalPhraseStats',dialogueVocabProgress:'apsDialogueVocabProgressV1:hi',authChoice:'apsAuthChoiceMade',authProfile:'apsAuthProfile',selectedLanguage:'apsSelectedLanguage',downloadedLanguages:'apsDownloadedLanguages'};
...
const state={
  ...
  vocabSettings:{rate:.9,rateEn:.9,rateHi:.9,translationDelay:1.5,gap:2,repeat:1,order:'sequential',reading:'en-hi',examples:true,hideEnglish:false,hideHindi:false,voiceEn:'',voiceHi:'',dialogueVoiceEnS1:'',dialogueVoiceEnS2:'',dialogueVoiceHiS1:'',dialogueVoiceHiS2:''},
...
function renderModal(){
  ...
  if(state.modal.type==='app-settings'){
    ...
    <div class="voice-settings-section language-settings-section"><h3>Preparation language</h3><p>Your progress is kept separately for each language.</p><div class="current-language-row"><span class="language-native">${esc(state.languagePack?.nativeName||'')}</span><div><b>${esc(state.languagePack?.name||'No language selected')}</b><small>${esc(state.languagePack?.pairLabel||'')}</small></div>${button('Manage languages','manage-languages','secondary')}</div></div>
