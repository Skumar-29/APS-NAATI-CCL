...
const LANGUAGE_META_FALLBACK=Object.freeze({
  hi:{name:'Hindi',nativeName:'हिन्दी',locale:'hi-IN',script:'devanagari'},
  pa:{name:'Punjabi',nativeName:'ਪੰਜਾਬੀ',locale:'pa-IN',script:'gurmukhi'},
  ur:{name:'Urdu',nativeName:'اردو',locale:'ur-PK',script:'arabic'},
  ne:{name:'Nepali',nativeName:'नेपाली',locale:'ne-NP',script:'devanagari'},
  gu:{name:'Gujarati',nativeName:'ગુજરાતી',locale:'gu-IN',script:'gujarati'},
  bn:{name:'Bengali',nativeName:'বাংলা',locale:'bn-BD',script:'bengali'},
  ta:{name:'Tamil',nativeName:'தமிழ்',locale:'ta-IN',script:'tamil'},
  te:{name:'Telugu',nativeName:'తెలుగు',locale:'te-IN',script:'telugu'},
  zh:{name:'Mandarin',nativeName:'中文',locale:'zh-CN',script:'han'},
  ar:{name:'Arabic',nativeName:'العربية',locale:'ar-SA',script:'arabic'}
});
function activeLanguageId(){return String(state?.selectedLanguage||state?.languagePack?.id||localStorage.getItem(storageKeys.selectedLanguage)||'hi').toLowerCase();}
function languageInfo(id=activeLanguageId()){
  const code=String(id||'hi').toLowerCase();
  const catalog=state?.languageCatalog?.find?.(x=>String(x.id).toLowerCase()===code)||{};
  return {...(LANGUAGE_META_FALLBACK[code]||{}),...catalog,id:code};
}
function targetLanguageName(id=activeLanguageId()){return languageInfo(id).name||String(id||'').toUpperCase();}
function targetLanguageLocale(id=activeLanguageId()){return languageInfo(id).targetLocale||languageInfo(id).locale||String(id||'hi');}
function scopedProgressKey(base,id=activeLanguageId()){
  const code=String(id||'hi').toLowerCase();
  return code==='hi'?base:`${base}:${code}`;
}
function setLanguageScopedStorageKeys(languageId){
  const code=String(languageId||'hi').toLowerCase();
  const names={
    vocabStatus:'apsFinalVocabStatus',vocabSettings:'apsFinalVocabSettings',vocabResume:'apsFinalVocabResume',attempts:'apsFinalAttempts',lesson:'apsFinalLesson',mistakes:'apsFinalMistakes',phraseStats:'apsFinalPhraseStats',myVocabs:'apsMyVocabsV1',practiceDaily:'apsPracticeDailyV9',recallProgress:'apsRecallProgressV9',recallSettings:'apsRecallSettingsV9'
  };
  for(const [field,base] of Object.entries(names)){
    if(field==='myVocabs')storageKeys[field]=code==='hi'?HINDI_LEGACY_PROGRESS_KEYS.myVocabs:`${base}:${code}`;
    else storageKeys[field]=code==='hi'?(HINDI_LEGACY_PROGRESS_KEYS[field]||base):`${base}:${code}`;
  }
  storageKeys.dialogueVocabProgress=code==='hi'?HINDI_LEGACY_PROGRESS_KEYS.dialogueVocabProgress:`apsDialogueVocabProgressV1:${code}`;
  return storageKeys;
}
...
const state={
  ...
  dialogue:null,dialogueMode:'learning',segmentIndex:0,responses:[],completed:new Set(),repeats:0,dialogueStartedAt:0,dialogueCompletedAt:0,
...
const DEFAULT_VOCAB_SETTINGS=JSON.parse(JSON.stringify(state.vocabSettings));
...
const formatElapsedTime=seconds=>{const total=Math.max(0,Math.round(Number(seconds)||0)),mins=Math.floor(total/60),secs=total%60;return `${mins}:${String(secs).padStart(2,'0')}`;};
...
async function loadLanguagePack(languageId){
  ...
  setLanguageScopedStorageKeys(languageId);
  ...
  if(previousLanguage&&previousLanguage!==languageId){
    state.dialogue=null;state.responses=[];state.completed=new Set();state.segmentIndex=0;state.repeats=0;state.feedback=null;state.report=null;state.mock=null;state.mockPair=null;
    state.playerStatus='ready';state.recordingUrl='';state.recordingBlob=null;state.recordingId='';state.transcript='';state.transcriptInterim='';state.practiceLibrary='verified';
    Object.assign(state.practice,{topic:'all',difficulty:'all',query:'',review:'study',completion:'all'});
    state.learn.revealed=new Set();state.vocabPlayer={...state.vocabPlayer,queue:[],index:0,playing:false,token:(Number(state.vocabPlayer?.token)||0)+1,revealCurrent:false};
  }
...
function dialogueStatsMap(){
  ...
  getJSON(storageKeys.attempts,[]).filter(a=>a.finished&&a.dialogueId).forEach(a=>{
    const x=m[a.dialogueId]||{practiceCount:0,lastPractisedAt:'',bestLow:null,bestHigh:null,bestTimeSeconds:null,lastTimeSeconds:null,modes:{}};
    ...
    const elapsed=Number(a.elapsedSeconds)||0;if(elapsed>0){x.lastTimeSeconds=elapsed;x.bestTimeSeconds=x.bestTimeSeconds===null?elapsed:Math.min(x.bestTimeSeconds,elapsed);}
    m[a.dialogueId]=x;
  });
  return m;
}
...
function progress(){
  ...
  <section class="card"><small>DIALOGUE COMPLETION RECORDS</small><h3>Which dialogues you have done and how many times</h3>${completedDialogues.length?`<div class="completion-records">${completedDialogues.map(d=>{const r=dialogueRecords[d.id];return `<div><div><strong>${esc(d.title)}</strong><small>${topicLabels[d.topic]||'Community'} · last practised ${new Date(r.lastPractisedAt).toLocaleString()}</small></div><span class="record-count">${r.practiceCount}× practised</span>${r.bestLow!==null?`<em>Best ${r.bestLow}–${r.bestHigh}/45</em>`:''}${r.bestTimeSeconds?`<em>Fastest ${formatElapsedTime(r.bestTimeSeconds)}</em>`:''}<button data-action="open-dialogue" data-id="${d.id}" data-mode="practice">Practise again</button></div>`;}).join('')}</div>`:'<div class="empty">No dialogue has been completed yet. Remaining dialogues are visible under Practice → Remaining.</div>'}</section>
...
function lessonNarration(slide){if(activeLanguageId()!=='hi')return slide.narrationEn;return state.lesson.lang==='en'?slide.narrationEn:state.lesson.lang==='hi'?slide.narrationHi:`${slide.narrationEn} ${slide.narrationHi}`;}
function lessonQuiz(){
...
function recordingPanel(response,seg){
  ...
  if(state.recording){
    return `<section class="recording-panel active">${skipRecordingControl}
      ...
      <div>
        <h3>Recording your ${seg.sourceLanguage==='en'?targetLanguageName():'English'} interpretation</h3>
...
function renderModal(){
  ...
  if(state.modal.type==='manage-languages')return `<div class="modal-backdrop"><div class="modal settings-modal"><button class="modal-close" data-action="close-modal">×</button><small>MY LANGUAGES</small><h2>Manage preparation languages</h2><p>Available language packs use the same app while keeping learning records separate for each language.</p><div class="language-list compact">${state.languageCatalog.map(x=>`<button class="language-option ${x.id===state.selectedLanguage?'selected':''} ${x.status==='available'?'available':'coming'}" data-action="switch-language" data-language="${esc(x.id)}" ${x.status==='available'?'':'disabled'}><span class="language-native">${esc(x.nativeName)}</span><span><b>${esc(x.name)}</b><small>${esc(x.pairLabel||'')}</small></span><em>${x.id===state.selectedLanguage?(x.pilot?'Active · Pilot':'Active'):x.status==='available'?(x.pilot?'Pilot':'Available'):'Coming soon'}</em></button>`).join('')}</div>${button('Done','close-modal','primary wide')}</div></div>`;
  if(state.modal.type==='app-settings'){
    const counts=voiceCounts();
    return `<div class="modal-backdrop"><div class="modal settings-modal app-settings-modal"><button class="modal-close" data-action="close-modal">×</button><div class="settings-heading"><div><small>SETTINGS</small><h2>Settings</h2></div><span>${counts.en} EN · ${counts.target} ${esc(activeLanguageId().toUpperCase())}</span></div>
    ...
    <div class="voice-settings-section"><h3>Learning voices</h3>${voiceSettingRow('English learning voice','voiceEn','en',state.vocabSettings.voiceEn,'general')}${voiceSettingRow(`${targetLanguageName()} learning voice`,'voiceHi',activeLanguageId(),state.vocabSettings.voiceHi,'general')}</div>
    ...
    <div class="voice-settings-section"><h3>Dialogue voices</h3>${voiceSettingRow('English · Speaker 1','dialogueVoiceEnS1','en',state.vocabSettings.dialogueVoiceEnS1,'S1')}${voiceSettingRow('English · Speaker 2','dialogueVoiceEnS2','en',state.vocabSettings.dialogueVoiceEnS2,'S2')}${voiceSettingRow(`${targetLanguageName()} · Speaker 1`,'dialogueVoiceHiS1',activeLanguageId(),state.vocabSettings.dialogueVoiceHiS1,'S1')}${voiceSettingRow(`${targetLanguageName()} · Speaker 2`,'dialogueVoiceHiS2',activeLanguageId(),state.vocabSettings.dialogueVoiceHiS2,'S2')}</div>
...
function availableVoices(lang){
  const prefix=String(lang||'en').toLowerCase().split(/[-_]/)[0]||'en',seen=new Set();
...
function voiceCounts(){const target=activeLanguageId();return {en:availableVoices('en').length,target:availableVoices(target).length,targetId:target};}
...
function getVoice(lang,name){const voices=speechVoices(),prefix=String(lang||'en').toLowerCase().split(/[-_]/)[0]||'en';return voices.find(v=>v.name===name)||voices.find(v=>(v.lang||'').toLowerCase().replace('_','-').startsWith(prefix))||null;}
function selectedVoiceName(lang,speaker='general'){const target=String(lang||'en').split(/[-_]/)[0]!=='en';if(speaker==='S1')return state.vocabSettings[target?'dialogueVoiceHiS1':'dialogueVoiceEnS1']||state.vocabSettings[target?'voiceHi':'voiceEn'];if(speaker==='S2')return state.vocabSettings[target?'dialogueVoiceHiS2':'dialogueVoiceEnS2']||state.vocabSettings[target?'voiceHi':'voiceEn'];return state.vocabSettings[target?'voiceHi':'voiceEn'];}
function speak(text,lang='en',rate=.9,token=null,speaker='general'){return new Promise(resolve=>{if(!('speechSynthesis'in window)||!text)return resolve();const code=String(lang||'en').toLowerCase().split(/[-_]/)[0]||'en',u=new SpeechSynthesisUtterance(text);u.lang=code==='en'?'en-AU':(languageInfo(code).targetLocale||languageInfo(code).locale||code);u.rate=Number(rate)||1;u.voice=getVoice(code,selectedVoiceName(code,speaker));u.onend=()=>resolve();u.onerror=()=>resolve();speechSynthesis.speak(u);});}
function vocabRate(lang){return Number(String(lang||'en').split(/[-_]/)[0]==='en'?state.vocabSettings.rateEn:state.vocabSettings.rateHi)||Number(state.vocabSettings.rate)||.9;}
...
async function speakLearningPair(english,hindi,token=null){
  const target=activeLanguageId(),mode=state.vocabSettings.reading==='both'?'en-hi':state.vocabSettings.reading,translationDelay=Number(state.vocabSettings.translationDelay)||0;
  if(mode==='english'){await speak(english,'en',vocabRate('en'),token);return vocabTokenActive(token);}
  if(mode==='hindi'){await speak(hindi,target,vocabRate(target),token);return vocabTokenActive(token);}
  const first=mode==='hi-en'?[target,hindi]:['en',english],second=mode==='hi-en'?['en',english]:[target,hindi];
...
function synonymSpeechLanguage(text){
  const value=String(text||''),target=activeLanguageId();
  if(target==='hi'&&/[\u0900-\u097F]/.test(value))return 'hi';
  if(target==='pa'&&/[\u0A00-\u0A7F]/.test(value))return 'pa';
  if(target==='gu'&&/[\u0A80-\u0AFF]/.test(value))return 'gu';
  if(target==='bn'&&/[\u0980-\u09FF]/.test(value))return 'bn';
  if((target==='ur'||target==='ar')&&/[\u0600-\u08FF]/.test(value))return target;
  return 'en';
}
...
async function playLesson(){
  ...
  while(state.lesson.playing&&state.overlay==='lesson'&&!state.lesson.quiz){
    ...
    speechSynthesis.cancel();
    if(state.lesson.lang==='bilingual'&&activeLanguageId()==='hi'){
    ...
    }else if(state.lesson.lang==='hi'&&activeLanguageId()==='hi'){
      await speak(slide.narrationHi,'hi',state.lesson.rate);
    }else{
      await speak(slide.narrationEn,'en',state.lesson.rate);
    }
...
async function finishRecording(){
  ...
  if(getActiveSegments().every((_,i)=>Boolean(state.responses[i])))state.dialogueCompletedAt=Date.now();
...
function assessAndSaveDialogue(){
  ...
  const finishedMs=state.dialogueCompletedAt||Date.now(),startedMs=state.dialogueStartedAt||finishedMs,elapsedSeconds=Math.max(0,(finishedMs-startedMs)/1000);
  const attempt={id:`attempt-${Date.now()}`,languageId:activeLanguageId(),dialogueId:d.id,title:d.title,mode:state.dialogueMode,startedAt:new Date(startedMs).toISOString(),finishedAt:new Date(finishedMs).toISOString(),finished:true,elapsedSeconds,repeats:state.repeats,responses:state.responses.map(r=>({...r,recordingUrl:''})),report};
...
function saveMistakes(attempt,d,segments){let m=getJSON(storageKeys.mistakes,[]);attempt.responses.forEach((r,i)=>{if(['review','major'].includes(r.assessment.status))m.push({id:`mistake-${Date.now()}-${i}`,languageId:activeLanguageId(),dialogueId:d.id,dialogueTitle:d.title,segmentId:segments[i].id,segmentNumber:i+1,status:r.assessment.status,review:r.assessment.review,mastered:false,createdAt:new Date().toISOString()});});setJSON(storageKeys.mistakes,m.slice(-300));}
function startMock(){const pair=currentMockPair();if(pair.length<2)return showToast('Two dialogues are required');state.mock={ids:pair.map(d=>d.id),current:0,attempts:[],startedAt:Date.now()};openDialogue(state.mock.ids[0],'mock');}
function finishMockDialogue(attempt){state.mock.attempts.push(attempt);if(state.mock.current===0){state.mock.current=1;openDialogue(state.mock.ids[1],'mock');state.mock=Object.assign(state.mock||{}, {current:1});showToast('Dialogue 1 complete. Dialogue 2 starts now.');return;}const pass=APSScoring.mockPass(state.mock.attempts[0].report,state.mock.attempts[1].report),mockFinishedAt=state.dialogueCompletedAt||Date.now(),totalElapsedSeconds=Math.max(0,(mockFinishedAt-(Number(state.mock.startedAt)||mockFinishedAt))/1000);state.report={type:'mock',dialogues:state.mock.attempts,pass,totalElapsedSeconds};state.overlay='report';render();}
...
app.addEventListener('click',async e=>{
  ...
  else if(a==='switch-language'){const id=el.dataset.language;if(id!==state.selectedLanguage){await chooseLanguage(id);state.modal=null;state.tab='home';render();}else{state.modal=null;render();}}
  ...
  else if(a==='preview-voice'){speechSynthesis.cancel();const lang=el.dataset.lang||'en',speaker=el.dataset.speaker||'general',text=lang==='pa'?'ਸਤ ਸ੍ਰੀ ਅਕਾਲ। ਇਹ ਤੁਹਾਡੀ ਚੁਣੀ ਹੋਈ ਪੰਜਾਬੀ ਆਵਾਜ਼ ਦਾ ਨਮੂਨਾ ਹੈ।':lang==='hi'?'नमस्ते। यह आपकी चुनी हुई हिन्दी आवाज़ का नमूना है।':'Hello. This is a preview of your selected voice.';await speak(text,lang,.9,null,speaker);}
  ...
  else if(a==='play-sample-answer'){const seg=getActiveSegments()[state.segmentIndex];if(seg){speechSynthesis.cancel();await speak(seg.sampleAnswer||seg.model,seg.sourceLanguage==='en'?activeLanguageId():'en',state.dialogueSettings.rate,null,seg.speaker||'general');}}
  else if(a==='record-again'){state.responses[state.segmentIndex]=null;state.completed.delete(getActiveSegments()[state.segmentIndex].id);state.dialogueCompletedAt=0;playDialogueSegment(false);}
