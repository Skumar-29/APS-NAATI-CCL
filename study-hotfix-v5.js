...
(() => {
  ...
  const BUILD_VERSION = 'github-study-ready-2026-08-07-v5';
  ...
  function setSearchButtonsExpanded(expanded) {
    qsa('[data-action="global-search"]').forEach(button => {
      button.setAttribute('aria-expanded', String(Boolean(expanded)));
    });
  }

  function positionSearchPanel(trigger) {
    const shell = qs('#apsStudySearchShell');
    const panel = qs('#apsStudySearchPanel', shell || document);
    if (!shell || !panel) return;

    const viewportPadding = 14;
    const panelWidth = Math.min(460, window.innerWidth - (viewportPadding * 2));
    let top = 74;
    let left = window.innerWidth - panelWidth - viewportPadding;
    ...
    shell.style.setProperty('--aps-search-top', `${Math.round(top)}px`);
    shell.style.setProperty('--aps-search-left', `${Math.round(left)}px`);
    shell.style.setProperty('--aps-search-width', `${Math.round(panelWidth)}px`);
  }
  ...
  function ensureSearchShell() {
    ...
    qs('#apsStudySearchClose', shell).addEventListener('click', closeSearchPanel);
  ...
  function openSearchPanel(trigger) {
    const shell = ensureSearchShell();
    positionSearchPanel(trigger);
    shell.hidden = false;
    setSearchButtonsExpanded(true);
    window.setTimeout(() => qs('#apsStudySearchInput', shell)?.focus(), 0);
  }
  ...
  // Global Search stays in the app header/toolbars instead of floating over Settings.
  document.addEventListener('click', event => {
    const trigger = event.target.closest('[data-action="global-search"]');
    if (trigger) {
      event.preventDefault();
      const shell = ensureSearchShell();
      if (!shell.hidden) closeSearchPanel();
      else openSearchPanel(trigger);
      return;
    }
    ...
    if (
      shell &&
      !shell.hidden &&
      !event.target.closest('#apsStudySearchPanel')
    ) {
      closeSearchPanel();
    }
  });
  ...
  window.addEventListener('resize', () => {
    ...
    const trigger = qs('[data-action="global-search"][aria-expanded="true"]');
    positionSearchPanel(trigger);
  });
