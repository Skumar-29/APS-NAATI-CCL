(function(){
'use strict';
const VERSION='My Vocabs V19.2';
const ONLINE_CACHE_KEY='apsMyVocabsOnlineAssistV2:hi';
...
async function translateOnline(text,target='hi'){
  ...
  // Google-style online translation is the primary path. We deliberately do not
  // accept looser crowd/memory translation services because V19.1 showed that
  // they can return unrelated phrase fragments for simple words.
  try{
...
async function onlineDictionary(term){
  const q=String(term||'').trim();if(!q||q.split(/\s+/).length>4)return {definition:'',example:'',synonyms:[]};
  try{
    const d=await fetchJson(`https://api.dictionaryapi.dev/api/v2/entries/en/${encodeURIComponent(q)}`);
    for(const entry of (Array.isArray(d)?d:[]))for(const meaning of (entry.meanings||[])){
      const senseSynonyms=cleanList([...(meaning.synonyms||[])]);
      for(const def of (meaning.definitions||[])){
        const definition=String(def.definition||'').trim();const example=String(def.example||'').trim();
        const synonyms=cleanList([...senseSynonyms,...(def.synonyms||[])]).filter(x=>normaliseSearchText(x)!==normaliseSearchText(q)).slice(0,6);
        if(definition||example||synonyms.length)return {definition,example,partOfSpeech:meaning.partOfSpeech||'',synonyms};
      }
      if(senseSynonyms.length)return {definition:'',example:'',partOfSpeech:meaning.partOfSpeech||'',synonyms:senseSynonyms.slice(0,6)};
    }
  }catch{}
...
async function onlineLookupProposal(term){
  ...
  try{
    const [hindi,dict]=await Promise.all([translateOnline(q,'hi'),onlineDictionary(q)]);
    const syn=cleanList([...(local.suggestedSynonyms||[]),...(dict.synonyms||[])]).filter(x=>normaliseSearchText(x)!==normaliseSearchText(q)).slice(0,6);
    ...
    const data={
      ...
      suggestedSynonyms:syn,
      exampleEnglish:exampleEnglish||local.exampleEnglish||'',
      exampleHindi:exampleHindi||local.exampleHindi||'',
      topic:local.topic||'community',lookupSource:'Google-style online translation + dictionary',online:true
    };
...
async function applyLookup(record,{fillMissingOnly=true,replaceAuto=false}={}){
  ...
  const setIfAllowed=(field,value)=>{if(!value)return;if(record.manualFields[field])return;if(fillMissingOnly&&!replaceAuto&&String(record[field]||'').trim())return;record[field]=value;};
  setIfAllowed('hindi',p.hindi||'');
  if(!record.manualFields.suggestedSynonyms)record.suggestedSynonyms=p.suggestedSynonyms||[];
...
async function autoFillRecord(id,{showMessage=false,replaceAuto=true}={}){
  ...
  r.lookupSource=navigator.onLine?'Looking up online…':'Checking APS offline fallback…';upsert(r);updateRowDom(r);
...
const V192_REPAIR_KEY='apsMyVocabsV192AutoRepair:hi';
async function refreshLegacyAutoRowsOnce(){
  ...
  const targets=rows().filter(r=>String(r.english||'').trim()&&(r.lastLookupOnline||/Online translation|MYMEMORY/i.test(String(r.lookupSource||'')))).slice(0,50);
...
console.info(`${VERSION} loaded · Google-style auto translation on Enter · compact focus workspace · master APS content remains separate.`);
})();
