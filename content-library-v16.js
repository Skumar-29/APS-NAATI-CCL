'use strict';
(() => {
  const VERSION='content-library-v16';
  ...
  const emptyOwner=()=>({schemaVersion:'1.0',contentVersion:'16.0.0',language:state.selectedLanguage||'hi',updatedAt:null,vocabulary:{},phrases:{},dialogues:{}});
  ...
  function applyOwnerContent(){
    ...
    vocabMap.forEach(record=>{
      ...
      const common={
        ...
        exampleEnglish:record.exampleEnglish||record.sourceExampleEnglish||'',exampleHindi:record.exampleHindi||'',notes:record.notes||'',qualityStatus:record.reviewStatus==='published'?'owner-published-v16':record.reviewStatus==='reviewed'?'owner-reviewed-v16':record.qualityStatus,
        status:record.status||record.reviewStatus||'owner-v16'
      };
      if(record.allocations?.core)core.push({...common,itemType:'word'});
      if(record.allocations?.general)general.push({...common,itemType:'general',kind:'general-vocabulary',qualityStatus:common.qualityStatus||'owner-reviewed-v16',qualityLabel:'Owner reviewed'});
    ...
    phraseMap.forEach(record=>{
      if(record.archived)return;
      const common={id:record.id,topic:record.topic||'community',english:record.english||'',hindi:record.hindi||'',acceptedHindi:[...(record.acceptedHindi||[])],exampleEnglish:record.exampleEnglish||'',exampleHindi:record.exampleHindi||'',notes:record.notes||'',status:record.reviewStatus||record.status||'owner-v16',itemType:'phrase'};
  ...
  // Keep the working V15 language-pack flow and layer V16 content after it.
  const v15LoadLanguagePack=loadLanguagePack;
  loadLanguagePack=async function v16LoadLanguagePack(languageId){
  ...
  vocabPlayerOverlay=function v16VocabPlayerOverlay(){
  ...
  speakVocabItem=async function v16SpeakVocabItem({autoplay=false}={}){
  ...
  function makeVocabDraft(id){
    ...
    return row?clone(row):{id:`own-vocab-hi-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,6)}`,english:'',hindi:'',acceptedHindi:[],exampleEnglish:'',exampleHindi:'',notes:'',topic:'community',allocations:{core:false,general:false,dialogues:[]},reviewStatus:'draft',archived:false,source:'owner-v16'};
  }
  function makePhraseDraft(id){
    const row=id?phraseCatalog().get(id):null;
    return row?clone(row):{id:`own-phrase-hi-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,6)}`,english:'',hindi:'',acceptedHindi:[],exampleEnglish:'',exampleHindi:'',notes:'',topic:'community',allocations:{main:true,dialogues:[]},reviewStatus:'draft',archived:false,source:'owner-v16'};
  }
  ...
  function renderVocabEditor(){
    ensureSelectedVocab();const d=state.v16Studio.vocabDraft,dupes=duplicateMatches(d,'vocab');
    return `<section class="v16-editor"><header><div><small>VOCABULARY RECORD</small><h3>${d.english?esc(d.english):'New vocabulary'}</h3></div>${statusBadge(d.reviewStatus)} </header>${dupes.length?`<div class="v16-duplicate-warning"><b>Similar English term already exists.</b> ${dupes.map(x=>`${esc(x.english)} → ${esc(x.hindi)}`).join(' · ')}. Keep separate only when it represents a different meaning/sense.</div>`:''}<div class="v16-form-grid"><label>English word / term<input id="v16VocabEnglish" value="${esc(d.english||'')}"></label><label>Hindi meaning<input id="v16VocabHindi" value="${esc(d.hindi||'')}"></label><label>Topic<select id="v16VocabTopic">${topicSelect(d.topic||'community')}</select></label><label>Stable ID<input value="${esc(d.id)}" disabled></label><label class="wide">Alternate Hindi meanings / synonyms <span>one per line</span><textarea id="v16VocabAlternatives" rows="3">${esc((d.acceptedHindi||[]).join('\n'))}</textarea></label><label class="wide">English example<textarea id="v16VocabExampleEn" rows="2">${esc(d.exampleEnglish||'')}</textarea></label><label class="wide">Hindi example<textarea id="v16VocabExampleHi" rows="2">${esc(d.exampleHindi||'')}</textarea></label><label class="wide">Owner notes<textarea id="v16VocabNotes" rows="2">${esc(d.notes||'')}</textarea></label></div><section class="v16-allocations"><h4>Allocate this vocabulary</h4><div class="v16-primary-alloc"><label><input id="v16VocabCore" type="checkbox" ${d.allocations?.core?'checked':''}> Core Vocabulary</label><label><input id="v16VocabGeneral" type="checkbox" ${d.allocations?.general?'checked':''}> General Vocabs</label></div><div class="v16-selected-alloc"><b>Dialogue allocations:</b> ${(d.allocations?.dialogues||[]).length?(d.allocations.dialogues||[]).map(id=>`<span>${esc(dialogueName(id))}</span>`).join(''):'<em>None</em>'}</div><label class="search v16-allocation-search"><span>⌕</span><input id="v16VocabDialogueSearch" placeholder="Find Dialogue 23, dentist, health…" value="${esc(state.v16Studio.vocabAllocSearch||'')}"></label><div id="v16VocabDialogueOptions" class="v16-dialogue-options">${allocationOptions(d.allocations?.dialogues||[],state.v16Studio.vocabAllocSearch)}</div></section><footer class="v16-editor-actions">${button(d.archived?'Restore item':'Archive item','v16-toggle-vocab-archive',d.archived?'secondary':'danger')}${button('Revert local change','v16-revert-vocab','secondary')}${button('Save Draft','v16-save-vocab-draft','secondary')}${button('Mark Reviewed','v16-save-vocab-reviewed','primary')}</footer></section>`;
  }

  function renderPhraseEditor(){
    ensureSelectedPhrase();const d=state.v16Studio.phraseDraft,dupes=duplicateMatches(d,'phrase');
    return `<section class="v16-editor"><header><div><small>PHRASE RECORD</small><h3>${d.english?esc(d.english):'New phrase'}</h3></div>${statusBadge(d.reviewStatus)}</header>${dupes.length?`<div class="v16-duplicate-warning"><b>Similar phrase already exists.</b> ${dupes.map(x=>`${esc(x.english)} → ${esc(x.hindi)}`).join(' · ')}</div>`:''}<div class="v16-form-grid"><label>English phrase<input id="v16PhraseEnglish" value="${esc(d.english||'')}"></label><label>Hindi meaning<input id="v16PhraseHindi" value="${esc(d.hindi||'')}"></label><label>Topic<select id="v16PhraseTopic">${topicSelect(d.topic||'community')}</select></label><label>Stable ID<input value="${esc(d.id)}" disabled></label><label class="wide">Alternate Hindi meanings <span>one per line</span><textarea id="v16PhraseAlternatives" rows="3">${esc((d.acceptedHindi||[]).join('\n'))}</textarea></label><label class="wide">English example<textarea id="v16PhraseExampleEn" rows="2">${esc(d.exampleEnglish||'')}</textarea></label><label class="wide">Hindi example<textarea id="v16PhraseExampleHi" rows="2">${esc(d.exampleHindi||'')}</textarea></label><label class="wide">Owner notes<textarea id="v16PhraseNotes" rows="2">${esc(d.notes||'')}</textarea></label></div><section class="v16-allocations"><h4>Allocate this phrase</h4><div class="v16-primary-alloc"><label><input id="v16PhraseMain" type="checkbox" ${d.allocations?.main!==false?'checked':''}> Main Phrases library</label></div><div class="v16-selected-alloc"><b>Dialogue allocations:</b> ${(d.allocations?.dialogues||[]).length?(d.allocations.dialogues||[]).map(id=>`<span>${esc(dialogueName(id))}</span>`).join(''):'<em>None</em>'}</div><label class="search v16-allocation-search"><span>⌕</span><input id="v16PhraseDialogueSearch" placeholder="Find Dialogue 67, tenancy, medical…" value="${esc(state.v16Studio.phraseAllocSearch||'')}"></label><div id="v16PhraseDialogueOptions" class="v16-dialogue-options">${allocationOptions(d.allocations?.dialogues||[],state.v16Studio.phraseAllocSearch)}</div></section><footer class="v16-editor-actions">${button(d.archived?'Restore phrase':'Archive phrase','v16-toggle-phrase-archive',d.archived?'secondary':'danger')}${button('Revert local change','v16-revert-phrase','secondary')}${button('Save Draft','v16-save-phrase-draft','secondary')}${button('Mark Reviewed','v16-save-phrase-reviewed','primary')}</footer></section>`;
  }

  function countsForPublish(){
    const local=getLocalOwner(),dialogs=getDialogueOverrides();
    return {vocab:Object.keys(local.vocabulary||{}).length,phrases:Object.keys(local.phrases||{}).length,dialogs:Object.keys(dialogs||{}).length,drafts:[...Object.values(local.vocabulary||{}),...Object.values(local.phrases||{})].filter(x=>x.reviewStatus==='draft').length,reviewed:[...Object.values(local.vocabulary||{}),...Object.values(local.phrases||{})].filter(x=>x.reviewStatus==='reviewed').length};
  }
  ...
  function publishPayload(){
    ...
    return {schemaVersion:'1.0',contentVersion:'16.0.0-owner',language:state.selectedLanguage||'hi',updatedAt:nowIso(),vocabulary,phrases,dialogues:clone(getDialogueOverrides())};
  }
  ...
  function renderStudio(){
    ...
    return `<div class="modal-backdrop content-studio-backdrop"><div class="modal content-studio-modal v16-studio-modal"><button class="modal-close" data-action="v16-close-studio">×</button><div class="studio-heading"><div><small>OWNER / EDITOR TOOL · V16</small><h2>Content Library Studio</h2><p>Manage dialogues, vocabulary, phrases, examples, allocations and publishing without changing the learner account system or resetting progress.</p></div><span>Stable-ID content layer</span></div><nav class="v16-studio-tabs">${tabs.map(([id,label])=>`<button data-action="v16-studio-tab" data-id="${id}" class="${tab===id?'active':''}">${label}</button>`).join('')}</nav>${body}</div></div>`;
  }
  ...
  function saveRecord(kind,status){
    ...
    d.reviewStatus=status;d.updatedAt=nowIso();d.source='owner-v16';
    ...
    showToast(status==='reviewed'?'Saved and marked reviewed':'Draft saved');return true;
  }
  ...
  function toggleArchive(kind){
    ...
    d.archived=next;d.reviewStatus='reviewed';d.updatedAt=nowIso();const local=getLocalOwner();(isV?local.vocabulary:local.phrases)[d.id]=clone(d);saveLocalOwner(local);applyOwnerContent();render();showToast(next?'Archived locally':'Restored locally');
  }

  function exportBackup(){
    const payload={schemaVersion:'aps-content-library-backup-v16',exportedAt:nowIso(),language:state.selectedLanguage||'hi',localOwnerContent:getLocalOwner(),dialogueOverrides:getDialogueOverrides(),githubConfig:{owner:state.v16Github.owner||'',repo:state.v16Github.repo||'',branch:state.v16Github.branch||'main',path:state.v16Github.path||'content/owner-content-v16.json'}};
    const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`APS_NAATI_Content_Backup_V16_${new Date().toISOString().slice(0,10)}.json`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);showToast('Owner content backup exported');
  }

  async function importBackup(file){
    const data=JSON.parse(await file.text());if(data.schemaVersion!=='aps-content-library-backup-v16')throw new Error('This is not a V16 Content Library backup.');
    if(!confirm('Import this V16 backup and replace local owner edits on this device?'))return;
    ...
    applyOwnerContent();state.v16Studio.vocabDraft=null;state.v16Studio.phraseDraft=null;render();showToast('V16 owner content backup restored');
  }
  ...
  // Capture phase lets V16 return from the preserved V15 dialogue editor into the V16 studio.
  ...
  app.addEventListener('click',async event=>{
    ...
    else if(a==='v16-studio-tab'){event.preventDefault();if(state.v16Studio.tab==='vocabulary')captureVocabForm();if(state.v16Studio.tab==='phrases')capturePhraseForm();state.v16Studio.tab=el.dataset.id;render();}
    ...
    else if(a==='v16-new-vocab'){event.preventDefault();captureVocabForm();state.v16Studio.selectedVocabId='';state.v16Studio.vocabDraft=makeVocabDraft();render();}
    else if(a==='v16-select-vocab'){event.preventDefault();captureVocabForm();state.v16Studio.selectedVocabId=el.dataset.id;state.v16Studio.vocabDraft=makeVocabDraft(el.dataset.id);render();}
    ...
    else if(a==='v16-new-phrase'){event.preventDefault();capturePhraseForm();state.v16Studio.selectedPhraseId='';state.v16Studio.phraseDraft=makePhraseDraft();render();}
    else if(a==='v16-select-phrase'){event.preventDefault();capturePhraseForm();state.v16Studio.selectedPhraseId=el.dataset.id;state.v16Studio.phraseDraft=makePhraseDraft(el.dataset.id);render();}
  ...
  app.addEventListener('change',async event=>{
    ...
    if(t.id==='v16VocabFilter'){captureVocabForm();state.v16Studio.vocabFilter=t.value;render();}
      ...
      const d=state.v16Studio.tab==='vocabulary'?state.v16Studio.vocabDraft:state.v16Studio.phraseDraft;if(!d)return;const set=new Set(d.allocations?.dialogues||[]);if(t.checked)set.add(t.dataset.v16DialogueAllocation);else set.delete(t.dataset.v16DialogueAllocation);d.allocations.dialogues=[...set].sort();
    ...
    else if(t.id==='v16ImportFile'&&t.files?.[0]){try{await importBackup(t.files[0]);}catch(error){showToast(error.message||'Could not import V16 backup');}t.value='';}
  ...
  async function initialise(){
    ...
    if(!state.v16Base.captured){captureBase();await loadPublishedOwner();applyOwnerContent();initialiseExampleSettings();render();console.info(`${VERSION} loaded: unified owner content library, GitHub publishing, and independent example recall controls`);}
    return true;
  }
