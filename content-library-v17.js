...
(() => {
  ...
  const ownerPath=()=>state.selectedLanguage==='hi'?'./content/owner-content-v16.json':`./content/owner-content-v16-${state.selectedLanguage||'hi'}.json`;
  const githubOwnerPath=()=>state.selectedLanguage==='hi'?'content/owner-content-v16.json':`content/owner-content-v16-${state.selectedLanguage||'hi'}.json`;
  ...
  state.v16Github={...(getJSON(GITHUB_CONFIG_KEY,{})||{}),path:githubOwnerPath(),token:'',busy:false,status:'',lastCommit:''};
  ...
  async function loadPublishedOwner(){
    try{
      const r=await fetch(ownerPath(),{cache:'no-store'});
      if(!r.ok)throw new Error(`HTTP ${r.status}`);
      const published=safeOwner(await r.json());
      const publishedLanguage=String(published.language||'hi').toLowerCase();
      const activeLanguage=String(state.selectedLanguage||'hi').toLowerCase();
      if(publishedLanguage!==activeLanguage)throw new Error(`Published owner content language mismatch: ${publishedLanguage} ≠ ${activeLanguage}`);
      state.v16Published=published;
  ...
  loadLanguagePack=async function v17LoadLanguagePack(languageId){
    ...
    state.v16Published=emptyOwner();
    state.v16Studio.vocabDraft=null;state.v16Studio.phraseDraft=null;state.v16Studio.dialogueId='';state.v16Studio.selectedVocabId='';state.v16Studio.selectedPhraseId='';
    state.v16Github.path=githubOwnerPath();
    captureBase();
  ...
  function searchVocabRecords(){
    ...
    else if(filter==='example-review')rows=rows.filter(x=>!String(x.exampleEnglish||'').trim()||!String(x.exampleHindi||'').trim()||['source-reference-needs-review','teaching-definition-v19'].includes(String(x.exampleQuality||'')));
  ...
  function makeVocabDraft(id){
    ...
    return row?clone(row):{id:`own-vocab-${state.selectedLanguage||'hi'}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,6)}`,english:'',hindi:'',acceptedHindi:[],exampleEnglish:'',exampleHindi:'',notes:'',topic:'community',allocations:{core:false,general:false,dialogues:[]},reviewStatus:'draft',archived:false,source:'owner-v17'};
  }
  function makePhraseDraft(id){
    const row=id?phraseCatalog().get(id):null;
    return row?clone(row):{id:`own-phrase-${state.selectedLanguage||'hi'}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,6)}`,english:'',hindi:'',acceptedHindi:[],exampleEnglish:'',exampleHindi:'',notes:'',topic:'community',allocations:{main:true,dialogues:[]},reviewStatus:'draft',archived:false,source:'owner-v17'};
  }
  ...
  function validatePayload(payload){
    ...
    const check=(records,label)=>Object.values(records||{}).forEach(r=>{
      ...
      if(!String(r.hindi||'').trim())errors.push(`${label} ${r.id||'?'}: ${targetLanguageName()} is empty`);
  ...
  function saveRecord(kind,status){
    ...
    if(!d?.english?.trim()||!d?.hindi?.trim()){showToast(`English and ${targetLanguageName()} are both required`);return false;}
  ...
  async function importBackup(file){
    ...
    const backupLanguage=String(data.language||data.localOwnerContent?.language||'hi').toLowerCase(),activeLanguage=String(state.selectedLanguage||'hi').toLowerCase();
    if(backupLanguage!==activeLanguage)throw new Error(`This backup belongs to ${backupLanguage}; switch to that language before restoring it.`);
    ...
    if(data.githubConfig){state.v16Github={...state.v16Github,...data.githubConfig,path:githubOwnerPath(),token:''};setJSON(GITHUB_CONFIG_KEY,{...data.githubConfig,path:githubOwnerPath()});}
  ...
  app.addEventListener('input',event=>{
    ...
    if(t.id==='v16VocabSearch'){state.v16Studio.vocabQuery=t.value;const rows=searchVocabRecords(),list=document.querySelector('#v16VocabList'),summary=document.querySelector('#v16VocabSummary');if(summary)summary.textContent=`${rows.length.toLocaleString()} matching records · showing first ${Math.min(rows.length,80)}`;if(list)list.innerHTML=rows.slice(0,80).map(x=>`<button data-action="v16-select-vocab" data-id="${esc(x.id)}" class="${state.v16Studio.selectedVocabId===x.id?'active':''}"><span><b>${esc(x.english)}</b><em>${esc(x.hindi)}</em></span><small>${esc(allocLabel(x,'vocab'))}</small></button>`).join('')||'<div class="empty"><p>No vocabulary matches.</p></div>';}
      ...
      if(box)box.innerHTML=rows.map(x=>`<button data-action="v17-choose-merge" data-id="${esc(x.id)}"><span><b>${esc(x.english)}</b><em>${esc(x.hindi)}</em></span><small>${esc(allocLabel(x,kind))}</small></button>`).join('')||'<p>No likely duplicate found. Search by English or ${esc(targetLanguageName())}.</p>';
  ...
  async function initialise(){
    ...
    if(!(state.dialogueVocabMeta?.itemCount>0)||!(state.generalVocab?.length>0))return false;
